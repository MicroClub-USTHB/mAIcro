...
async def run_discord_listener(bot_token: str, channel_ids: list[str]) -> None:
    ...
    if not bot_token:
        logger.error(
            "[listener] DISCORD_BOT_TOKEN is not set — listener will not start"
        )
        return
    if not channel_ids:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            async def on_ready() -> None:
                logger.info(
                    "[listener] READY — logged in as %s (id=%s)",
                    client.user,
                    client.user.id,
                )
            ...
            async def on_raw_message_delete(
                ...
                message_id = str(payload.message_id)
                logger.debug(
                    "[listener] MESSAGE_DELETE channel=%s message_id=%s",
                    channel_id,
                    message_id,
                )
                await handle_message_delete(
            ...
            async def on_raw_message_edit(
                ...
                message_id = str(payload.message_id)
                logger.debug(
                    "[listener] MESSAGE_UPDATE channel=%s message_id=%s",
                    channel_id,
                    message_id,
                )
