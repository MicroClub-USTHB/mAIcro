...
def test_ingest_documents_bootstraps_missing_collection(monkeypatch):
    documents = [
        Document(page_content="Release planning", metadata={"source": "json_file"})
    ]
    bootstrapped = {}
    ...
    assert (
        getter.cache_clears == 2
    )  # cache cleared twice: once for index check, once after bootstrap
    assert working_store.added == documents


def test_ingest_documents_bootstraps_missing_collection_when_qdrant_returns_404(
    monkeypatch,
):
    documents = [
        Document(page_content="Release planning", metadata={"source": "json_file"})
...
def test_ingest_documents_ignores_missing_collection_during_duplicate_check(
    monkeypatch,
):
    documents = [
        Document(
...
def test_ingest_from_discord_collects_successes_and_permission_errors(monkeypatch):
    ...
    monkeypatch.setattr(
        ingestion, "get_last_ingested_message_id", lambda channel_id: None
    )
    monkeypatch.setattr(
        ingestion, "update_last_ingested_message_id", lambda channel_id, msg_id: None
    )
    ...
    async def fake_fetch_channel_messages(
        ...
        if channel_id == "denied":
            raise DiscordFetchError(
                channel_id=channel_id, status_code=403, message="forbidden"
            )
        return [
            {
                "id": "m1",
    ...
    monkeypatch.setattr(
        ingestion, "fetch_channel_messages", fake_fetch_channel_messages
    )
